...
class fixed_string_test : public beast::unit_test::suite
{
public:
    void
    testConstruct()
    ...
    testAccess()
    {
        ...
        }
        {
            fixed_string<3> s("123");
            string_view sv = s;
            BEAST_EXPECT(fixed_string<5>(sv) == "123");
        }
    }
    ...
    testOperations()
    {
        ...
        }
        {
            ...
            {
                fixed_string<6> s2("12345");
                s2.insert(6, fixed_string<3>("TUV"));
                fail("", __FILE__, __LINE__);
            }
            catch(std::out_of_range const&)
            {
        ...
        }
        {
            ...
            {
                fixed_string<6> s2("12345");
                s2.insert(6, fixed_string<3>("TUV"), 1, 2);
                fail("", __FILE__, __LINE__);
            }
            catch(std::out_of_range const&)
            {
    ...
    testToStaticString()
    {
        BEAST_EXPECT(to_fixed_string<long>(0) == "0");
        BEAST_EXPECT(to_fixed_string<long>(1) == "1");
        BEAST_EXPECT(to_fixed_string<long>(0xffff) == "65535");
        BEAST_EXPECT(to_fixed_string<long>(0x10000) == "65536");
        BEAST_EXPECT(to_fixed_string<long long>(0xffffffff) == "4294967295");

        BEAST_EXPECT(to_fixed_string<long>(-1) == "-1");
        BEAST_EXPECT(to_fixed_string<long>(-65535) == "-65535");
        BEAST_EXPECT(to_fixed_string<long>(-65536) == "-65536");
        BEAST_EXPECT(to_fixed_string<long long>(-4294967295ll) == "-4294967295");

        BEAST_EXPECT(to_fixed_string<unsigned long>(0) == "0");
        BEAST_EXPECT(to_fixed_string<unsigned long>(1) == "1");
        BEAST_EXPECT(to_fixed_string<unsigned long>(0xffff) == "65535");
        BEAST_EXPECT(to_fixed_string<unsigned long>(0x10000) == "65536");
        BEAST_EXPECT(to_fixed_string<unsigned long>(0xffffffff) == "4294967295");
    }
...
BEAST_DEFINE_TESTSUITE(boost,fixed_string,fixed_string);
